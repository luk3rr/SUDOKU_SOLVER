...
Options:
    ...
    -m, --memleak      Run Valgrind tests to check for memory leaks
...
def run_valgrind_tests():
    ...
    valgrind_program = f"{VALGRIND_COMMAND} {BIN_DIR}/{PROGRAM_NAME} > /dev/null"
    valgrind_tests = f"{VALGRIND_COMMAND} {BIN_DIR}/{UNIT_TEST_NAME}"

    print("Running Valgrind on program:")
    subprocess.run(valgrind_program, shell=True, check=True)

    print("Running Valgrind on unit tests:")
    subprocess.run(valgrind_tests, shell=True, check=True)
...
def main():
    ...
    parser.add_argument(
        ...
        "--memleak",
        action="store_true",
        help="Run Valgrind tests for memory leaks",
    )
    parser.add_argument(
